...
// Schema para reembolsos reclamados (para prevenir reclamos múltiples)
...
// Índice compuesto para evitar reclamos duplicados
...
app.post('/api/refunds/claim/daily', authMiddleware, async (req, res) => {
  try {
    ...
    try {
      ...
      // Registrar reembolso en la colección de reembolsos
      ...
      // Registrar en ClaimedRefund para trackear que ya fue reclamado
      try {
        ...
        console.log(`✅ Reembolso diario trackeado en MongoDB para ${username}`);
      } catch (trackError) {
        // Si ya existe, no es un error crítico
        if (trackError.code === 11000) {
          console.log(`⚠️ Reembolso diario ya estaba trackeado para ${username}`);
        } else {
          console.error('Error trackeando reembolso:', trackError);
        }
      }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        weekStart.setDate(now.getDate() - daysSinceMonday - 7); // Semana pasada
        ...
        const claimedRefund = new ClaimedRefund({
          ...
          periodEnd: new Date(weekStart.getTime() + 7 * 24 * 60 * 60 * 1000 - 1),
          claimedAt: new Date()
        });
        await claimedRefund.save();
        console.log(`✅ Reembolso semanal trackeado en MongoDB para ${username}`);
      } catch (trackError) {
        // Si ya existe, no es un error crítico
        if (trackError.code === 11000) {
          console.log(`⚠️ Reembolso semanal ya estaba trackeado para ${username}`);
        } else {
          console.error('Error trackeando reembolso:', trackError);
        }
      }
...
app.post('/api/refunds/claim/monthly', authMiddleware, async (req, res) => {
  try {
    ...
    try {
      ...
      try {
        const now = new Date();
        const monthStart = new Date(now.getFullYear(), now.getMonth() - 1, 1); // Mes pasado
        ...
        const claimedRefund = new ClaimedRefund({
          ...
          periodEnd: new Date(now.getFullYear(), now.getMonth(), 0, 23, 59, 59), // Último día del mes pasado
          claimedAt: new Date()
        });
        await claimedRefund.save();
        console.log(`✅ Reembolso mensual trackeado en MongoDB para ${username}`);
      } catch (trackError) {
        // Si ya existe, no es un error crítico
        if (trackError.code === 11000) {
          console.log(`⚠️ Reembolso mensual ya estaba trackeado para ${username}`);
        } else {
          console.error('Error trackeando reembolso:', trackError);
        }
      }
...
app.get('/api/admin/transactions', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    console.log(`📊 Cargando transacciones. Total en BD: ${transactions.length}`);
    ...
    transactions.forEach(t => {
      ...
      if (t.type === 'deposit') summary.deposits += amount;
      else if (t.type === 'withdrawal') summary.withdrawals += amount;
      else if (t.type === 'bonus') summary.bonuses += amount;
      else if (t.type === 'refund') summary.refunds += amount;
    });
    
    // Ordenar por fecha descendente
    transactions.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
    
    console.log(`📊 Transacciones filtradas: ${transactions.length}`, summary);
