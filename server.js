...
// ============================================
const { connectDB, isConnected, User, Message, ChatStatus, UserActivity, FireReward, Config, CustomCommand, Transaction, ExternalUser, SyncLog } = require('./database');
...
// Variables en memoria para fallback
let memoryUsers = [];
let memoryMessages = [];
...
// Funciones helpers con MongoDB + Fallback JSON
...
const saveMessages = async (messages) => {
  ...
  if (useMongoDB && isConnected()) {
    try {
      ...
      if (lastMessage) {
        await Message.findOneAndUpdate({ id: lastMessage.id }, lastMessage, { upsert: true });
      }
      return;
    } catch (error) {
...
function loadExternalUsers() {
  try {
    ...
    memoryExternalUsers = JSON.parse(fs.readFileSync(EXTERNAL_DB_FILE, 'utf8'));
  } catch (error) {
    memoryExternalUsers = [];
  }
...
function saveExternalUsers(users) {
  ...
  if (useMongoDB && isConnected()) {
    ExternalUser.deleteMany({}).then(() => {
      ExternalUser.insertMany(users).catch(err => console.error('Error syncing external users to MongoDB:', err));
    }).catch(err => console.error('Error clearing external users in MongoDB:', err));
  }
}
...
function loadChatStatus() {
  try {
    ...
    memoryChatStatus = JSON.parse(fs.readFileSync(CHAT_STATUS_FILE, 'utf8'));
  } catch {
    memoryChatStatus = {};
  }
...
function saveChatStatus(status) {
  ...
  // Sync to MongoDB in background
  if (useMongoDB && isConnected()) {
    ChatStatus.deleteMany({}).then(() => {
      const docs = Object.entries(status).map(([userId, data]) => ({ userId, ...data }));
      ChatStatus.insertMany(docs).catch(err => console.error('Error syncing chat status to MongoDB:', err));
    }).catch(err => console.error('Error clearing chat status in MongoDB:', err));
  }
}
...
function loadConfig() {
  try {
    ...
    memoryConfig = JSON.parse(fs.readFileSync(CONFIG_FILE, 'utf8'));
  } catch (error) {
...
function saveConfig(config) {
  ...
  try {
    ...
    // Sync to MongoDB in background
    if (useMongoDB && isConnected()) {
      Config.findOneAndUpdate({ key: 'system' }, { key: 'system', value: config }, { upsert: true })
        .catch(err => console.error('Error syncing config to MongoDB:', err));
    }
  } catch (error) {
