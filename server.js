...
// Función para cargar datos desde MongoDB a la caché
async function loadCacheFromMongoDB() {
  if (!mongoConnected || !User) {
    console.log('⚠️ MongoDB no conectado, usando archivos');
  ...
  try {
    ...
    if (mongoUsers && mongoUsers.length > 0) {
      ...
      console.log(`✅ ${usersCache.length} usuarios cargados de MongoDB`);
    } else {
      // Si no hay datos en MongoDB, cargar desde archivo
      usersCache = loadUsersFromFile();
      console.log(`📁 ${usersCache.length} usuarios cargados de archivo`);
    }
    
    // Cargar mensajes
    const mongoMessages = await Message.find().lean();
    if (mongoMessages && mongoMessages.length > 0) {
      messagesCache = mongoMessages;
      fs.writeFileSync(MESSAGES_FILE, JSON.stringify(messagesCache, null, 2));
      console.log(`✅ ${messagesCache.length} mensajes cargados de MongoDB`);
    } else {
      messagesCache = loadMessagesFromFile();
      console.log(`📁 ${messagesCache.length} mensajes cargados de archivo`);
    }
    
    cacheLoaded = true;
    console.log('✅ Cache cargada correctamente');
  } catch (err) {
    console.error('❌ Error cargando cache:', err.message);
    // Fallback a archivos
    usersCache = loadUsersFromFile();
    messagesCache = loadMessagesFromFile();
  }
...
// Funciones para leer desde archivo (fallback)
...
// Funciones helpers - USAN CACHE (síncronas para compatibilidad)
const loadUsers = () => {
  // Usar cache si está cargada
  if (cacheLoaded && usersCache.length > 0) {
    return [...usersCache]; // Retornar copia
  }
  // Fallback a archivo
...
const loadMessages = () => {
  // Usar cache si está cargada
  if (cacheLoaded && messagesCache.length > 0) {
    return [...messagesCache]; // Retornar copia
  }
  // Fallback a archivo
...
async function initializeData() {
  // ============================================
  // CARGAR CACHE DESDE MONGODB PRIMERO
  // ============================================
  await loadCacheFromMongoDB();
  
  // Ahora usar loadUsers() que leerá de la cache
  let users = loadUsers();
