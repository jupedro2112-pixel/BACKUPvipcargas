...
if (MONGODB_URI) {
  ...
  .catch(err => {
    console.log('⚠️ MongoDB no disponible, usando archivos JSON:', err.message);
    mongoConnected = false;
  });
} else {
  console.log('⚠️ MONGODB_URI no configurado, usando archivos JSON');
}
...
const userSchema = new mongoose.Schema({
  ...
  jugayganaUserId: String, jugayganaUsername: String, jugayganaSyncStatus: String,
  source: String, lastBonusDate: String, bonusDay: { type: Number, default: 0 },
  dailyWager: { type: Number, default: 0 }, weeklyWager: { type: Number, default: 0 },
...
const messageSchema = new mongoose.Schema({
  ...
  timestamp: String, read: { type: Boolean, default: false }
}, { timestamps: true });
...
const configSchema = new mongoose.Schema({
  ...
  cbu: { number: String, alias: String, bank: String, titular: String },
...
const saveUsers = (users) => {
  ...
  // Guardar en MongoDB (asíncrono, no bloquea)
  if (mongoConnected) {
    User.deleteMany({}).then(() => {
      if (users.length > 0) return User.insertMany(users);
    }).catch(err => console.error('Error guardando usuarios en MongoDB:', err.message));
  }
};
...
const saveMessages = (messages) => {
  ...
  if (mongoConnected) {
    Message.deleteMany({}).then(() => {
      if (messages.length > 0) return Message.insertMany(messages);
    }).catch(err => console.error('Error guardando mensajes en MongoDB:', err.message));
  }
};
...
try {
  ...
  console.error('Error creando archivo de transacciones:', error);
}
...
const saveTransactions = (transactions) => {
  ...
  if (mongoConnected) {
    Transaction.deleteMany({}).then(() => {
      if (transactions.length > 0) return Transaction.insertMany(transactions);
    }).catch(err => console.error('Error guardando transacciones en MongoDB:', err.message));
  }
};
...
async function initializeData() {
  ...
  if (mongoConnected) {
    console.log('🔄 Sincronizando datos desde MongoDB...');
    try {
      const mongoUsers = await User.find().lean();
      if (mongoUsers && mongoUsers.length > 0) {
        fs.writeFileSync(USERS_FILE, JSON.stringify(mongoUsers.map(u => ({...u, _id: undefined, __v: undefined})), null, 2));
        console.log(`✅ ${mongoUsers.length} usuarios cargados desde MongoDB`);
      }
      const mongoMessages = await Message.find().lean();
      if (mongoMessages && mongoMessages.length > 0) {
        fs.writeFileSync(MESSAGES_FILE, JSON.stringify(mongoMessages.map(m => ({...m, _id: undefined, __v: undefined})), null, 2));
      ...
      if (mongoTransactions && mongoTransactions.length > 0) {
        fs.writeFileSync(TRANSACTIONS_FILE, JSON.stringify(mongoTransactions.map(t => ({...t, _id: undefined, __v: undefined})), null, 2));
        console.log(`✅ ${mongoTransactions.length} transacciones cargadas desde MongoDB`);
      }
    } catch (err) {
      console.log('⚠️ Error sincronizando desde MongoDB:', err.message);
    }
  }
...
// Funciones de transacciones (ya definidas arriba con soporte MongoDB)
function saveTransaction(transaction) {
  try {
    ...
    // Guardar en MongoDB
    if (mongoConnected) {
      Transaction.create(newTransaction).catch(err => console.error('Error guardando transacción en MongoDB:', err.message));
    }
  } catch (error) {
