...
app.get('/api/auth/check-username', async (req, res) => {
  try {
    ...
    try {
      ...
      if (jgUser) {
        return res.json({ available: false, message: 'Usuario ya existe en JUGAYGANA' });
      }
    } catch (jgError) {
      // Si hay error consultando JUGAYGANA, solo verificamos local
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    // Verificar si el usuario existe en JUGAYGANA
    let jgUser = null;
    try {
    ...
    // Si existe en JUGAYGANA, usar la contraseña por defecto
    let finalPassword = password;
    if (jgUser) {
      finalPassword = 'asd123'; // Contraseña por defecto para usuarios de JUGAYGANA
    }
    ...
    const newUser = {
      ...
      balance: jgUser ? (jgUser.balance || 0) : 0,
      createdAt: new Date().toISOString(),
      lastLogin: null,
      isActive: true,
      jugayganaUserId: jgUser ? jgUser.id : null,
      jugayganaUsername: jgUser ? jgUser.username : null,
      jugayganaSyncStatus: jgUser ? 'linked' : 'pending'
    };
    ...
    res.status(201).json({
      ...
      user: {
        ...
        jugayganaLinked: !!jgUser,
        needsPasswordChange: jgUser && finalPassword === 'asd123'
      }
    });
  } catch (error) {
...
app.use('/api/admin/deposit', recordTransaction);
app.use('/api/admin/withdrawal', recordTransaction);
app.use('/api/admin/bonus', recordTransaction);
