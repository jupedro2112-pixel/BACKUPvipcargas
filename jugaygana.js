...
const client = axios.create({
  baseURL: API_URL,
  timeout: 60000, // Aumentado a 60 segundos para evitar timeouts
  httpsAgent,
  proxy: false,
  headers: {
  ...
  // Configuración para reintentar en caso de errores 502
  validateStatus: function (status) {
    return status >= 200 && status < 600; // Aceptar todos los status para manejar errores manualmente
  }
});
...
async function depositToUser(username, amount, description = '') {
  ...
  // Reintentar hasta 3 veces en caso de error 502
  let lastError = null;
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      console.log(`📝 Intento ${attempt}/3 de depósito...`);
      ...
      // Manejar error 502 - reintentar
      if (resp.status === 502 || resp.status === 503 || resp.status === 504) {
        console.log(`⚠️ Error ${resp.status} en intento ${attempt}, reintentando...`);
        lastError = `Error ${resp.status} - Gateway error`;
        if (attempt < 3) {
          await new Promise(r => setTimeout(r, 2000 * attempt)); // Espera progresiva
          continue;
        }
        return { success: false, error: 'El servidor de JUGAYGANA no responde. Intenta nuevamente en unos segundos.' };
      }
      ...
      if ((err.code === 'ECONNABORTED' || err.code === 'ETIMEDOUT' || err.message.includes('timeout')) && attempt < 3) {
...
async function withdrawFromUser(username, amount, description = '') {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      console.log(`📝 Intento ${attempt}/3 de retiro...`);
