...
let isProcessingDeposit = false; // Prevenir doble submit en depósitos
let activeToastTimeout = null; // Para limpiar toast anteriores
...
function showToast(message, type, toastId = null) {
    const container = document.getElementById('toastContainer');
    if (!container) return;
    
    // Si hay un toastId, remover toast anterior con el mismo ID
    if (toastId) {
    ...
    // Limpiar toast anteriores del mismo tipo (para depósitos)
    if (type === 'success' || type === 'warning') {
        const existingToasts = container.querySelectorAll('.toast');
        existingToasts.forEach(t => {
            if (t.textContent.includes('Depósito') || t.textContent.includes('Bonus')) {
                t.remove();
            }
        });
    }
...
async function selectConversation(userId, username) {
    ...
    // Limpiar mensajes anteriores inmediatamente para evitar replicación
    const messagesContainer = document.getElementById('messagesContainer');
    if (messagesContainer) {
        messagesContainer.innerHTML = '<div style="text-align: center; padding: 20px; color: #888;">Cargando mensajes...</div>';
    }
    
    // Reset hash para forzar re-renderizado
    lastMessagesHash = '';
    ...
    await loadMessages(userId, true); // Forzar carga
...
async function loadMessages(userId, force = false) {
    // Evitar cargar mensajes del usuario anterior si cambiamos rápidamente
    if (userId !== selectedUserId && !force) {
        console.log('Ignorando carga de mensajes - usuario cambiado');
        return;
    }
    
    // Evitar cargas simultáneas
    if (isLoadingMessages && !force) {
        console.log('Carga de mensajes en progreso, ignorando...');
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Verificar que el usuario no haya cambiado mientras cargábamos
            if (userId !== selectedUserId && !force) {
                console.log('Usuario cambió durante carga, ignorando mensajes');
                return;
            }
            
            // Calcular hash para evitar re-renderizado innecesario
            const messagesHash = messages.map(m => m.id + m.timestamp).join(',');
...
async function handleAdminDeposit(e) {
    ...
    if (isProcessingDeposit) {
        console.log('Depósito ya en proceso, ignorando...');
        return;
    }
