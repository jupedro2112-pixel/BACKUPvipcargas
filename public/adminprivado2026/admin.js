...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (closeDatabasePasswordModal) {
        closeDatabasePasswordModal.addEventListener('click', function() {
            // Volver a la sección de chats
            showSection('chats');
            document.querySelector('.nav-item[data-section="chats"]')?.classList.add('active');
            document.querySelector('.nav-item[data-section="database"]')?.classList.remove('active');
        });
    }
...
function configureUIBasedOnRole() {
    ...
    // Depositer: solo puede depositar, ver Abiertos y Cerrados (NO Pagos)
    if (role === 'depositor') {
        ...
        // Ocultar tab de Pagos
        if (tabPagosChats) tabPagosChats.style.display = 'none';
        
        // Mostrar tabs de Abiertos y Cerrados
        if (tabOpenChats) tabOpenChats.style.display = 'block';
        if (tabClosedChats) tabClosedChats.style.display = 'block';
    }
    ...
    if (role === 'withdrawer') {
        ...
        // Ocultar tabs de Abiertos y Cerrados
        if (tabOpenChats) tabOpenChats.style.display = 'none';
        if (tabClosedChats) tabClosedChats.style.display = 'none';
        
        // Mostrar tab de Pagos y cambiar estilo
        if (tabPagosChats) {
            tabPagosChats.style.display = 'block';
