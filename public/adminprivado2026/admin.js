...
function loadCommands() {
    ...
    // Combinar comandos predefinidos con personalizados
    ...
    commandsList.innerHTML = Object.keys(allCommands).map(function(cmd) {
        ...
        return '<div style="display: flex; justify-content: space-between; align-items: center; padding: 12px; background: rgba(0,0,0,0.3); border-radius: 8px; border: 1px solid rgba(212, 175, 55, 0.3);">' +
            '<div>' +
                '<div style="color: #d4af37; font-weight: bold; font-size: 14px;">' + cmd + '</div>' +
                '<div style="color: #888; font-size: 11px;">' + escapeHtml(command.description) + '</div>' +
            '</div>' +
            ...
            customCommands[name] = {
                ...
                action: function() {
                    if (type === 'bonus' && bonusPercent > 0) {
                        return '🎁 Bonus del ' + bonusPercent + '% aplicado';
                    }
                    return '✅ Comando ejecutado: ' + name;
                }
            };
            
            // Agregar a ADMIN_COMMANDS para que funcione inmediatamente
            ADMIN_COMMANDS[name] = customCommands[name];
