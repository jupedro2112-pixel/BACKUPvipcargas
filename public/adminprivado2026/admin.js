...
async function handleAdminDeposit(e) {
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            // Formato correcto del mensaje según si tiene bonus o no
            let depositMessage;
            if (hasBonus) {
...
function renderRefunds(refunds) {
    ...
    refunds.sort((a, b) => new Date(b.timestamp || b.date) - new Date(a.timestamp || a.date));
    ...
    tbody.innerHTML = refunds.map(function(r) {
        const date = new Date(r.timestamp || r.date).toLocaleString('es-AR');
        const typeLabel = typeLabels[r.type] || r.type;
        const typeColor = typeColors[r.type] || '#fff';
        
        // Asegurar que los valores sean números
        const amount = parseFloat(r.amount) || 0;
        const netAmount = parseFloat(r.netAmount) || 0;
        const deposits = parseFloat(r.deposits) || 0;
        const withdrawals = parseFloat(r.withdrawals) || 0;
        ...
        return '<tr style="border-bottom: 1px solid rgba(212, 175, 55, 0.1);">' +
            ...
            '<td style="padding: 12px; text-align: right; color: #00ff88; font-weight: bold;">$' + amount.toLocaleString() + '</td>' +
            '<td style="padding: 12px; text-align: right; color: #888;">$' + netAmount.toLocaleString() + '</td>' +
            '<td style="padding: 12px; text-align: right; color: #00a8ff;">$' + deposits.toLocaleString() + '</td>' +
            '<td style="padding: 12px; text-align: right; color: #ff4444;">$' + withdrawals.toLocaleString() + '</td>' +
        '</tr>';
    }).join('');
}
